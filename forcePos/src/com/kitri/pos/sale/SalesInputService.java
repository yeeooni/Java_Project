...
public class SalesInputService implements KeyListener, ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (ob == salebtn.sBtnPdChange) {
			int row = mainframe.viewSalesInput.table.getSelectedRow();
			if (row < 0) {
				JOptionPane.showMessageDialog(mainframe, "등록된 상품이 없습니다.", "상품 수정 오류", JOptionPane.WARNING_MESSAGE);
			...
			totalApply();
		} else if (ob == salebtn.sBtnAcencel) {
			int row = mainframe.viewSalesInput.table.getSelectedRow();
			if (row < 0) {
		...
		} else if (ob == salebtn.sBtnPdCancel) {
			int row = mainframe.viewSalesInput.table.getSelectedRow();
			if (row != -1) {
...
//----------------------------------------------------------------------------------------------
			...
			if (JOptionPane.showConfirmDialog(mainframe.payment_4, "결제하시겠습니까?", "결제확인", JOptionPane.OK_CANCEL_OPTION,
					JOptionPane.QUESTION_MESSAGE) == 0) {
				paymentProcess();
				JOptionPane.showMessageDialog(mainframe.payment_4, "결제가완료되었습니다.", "결제완료",
...
//		if()유저이름이 없다면 멤버쉽등록이 안됬으므로 포인트처리 생략
	...
	private void payNPrint() {
		mainframe.payment_4.taP4details.setText("판매코드 : " + salesInputDao.posDto.getSellId() + "\t판매일자 : "
...
//		int cnt = mainframe.viewSalesInput.table.getRowCount();
//		for(int i = 0; i < cnt ; i++) {
////			Vector<String> rows = new Vector<String>();
//			mainframe.payment_4.taP4details.append(
...
//					mainframe.payment_1.tfP1BeforePrice.getText());

		mainframe.payment_1.tfP1Afterprice.setText("");

//			int dis_int = (int) (salesInputDao.posDto.getDiscountPct() * 100);
//			mainframe.payment_1.tfP1DiscountPercent.setText(Integer.toString(dis_int));
//		mainframe.payment_1.tfP1Afterprice.setText(Integer.toString((int)(Integer.parseInt(
//				mainframe.payment_1.tfP1BeforePrice.getText()) * (1-dis_pct_dob))));
...
//@@@@@				salesList = salesInputDao.searchBy(mainframe.viewSalesInput.code_input.getText().trim().toUpperCase());
				listAdd(salesInputDao.searchBy(mainframe.viewSalesInput.code_input.getText().trim().toUpperCase()));
